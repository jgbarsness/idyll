...
to create a new collection file, you can just run ```idl``` as-is to use ```idl.txt```, or you can run ```idl -s [new_name]``` to set a new default name. running ```idl``` will print a header with useful information on files in your ```idl``` folder.
