...
to create a new collection file, run a command. default is ```idl.txt```, or you can run ```idl -s [new_name]``` to set a new default name. ```idl -s [name]``` can also be used to create a new collection file on next note. running ```idl -l``` will print a header with useful information on files in your ```idl``` folder.
...
2.) (optional) copy ```google_api_python_client-1.12.1.dist-info``` folder to ```[project]/bin/[os]/[idl]``` directory to allow for google drive support (note: not implemented in current builds / source. check issue #3 for more information)
